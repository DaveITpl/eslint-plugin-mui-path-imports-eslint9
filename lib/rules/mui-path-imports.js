...
import pathImportPaths from "./path-import-paths.json" assert { type: "json" }


/** 
 * @type {import('eslint').Rule.RuleModule}
 */
export const muiTopLevelImportsRule = {
  meta: {
    type: 'problem',
    docs: {
      description: "Enforce top-level imports for MUI components",
      recommended: false,
    },
    fixable: 'code',
    schema: [],
    messages: {
      muiTopLevelImportError: "!mui-toplevel-import is not allowed. Use top-level imports.",
    },
  },
  ...
  create(context) {
    return {
      ImportDeclaration(node) {
        ...
        context.report({
          node,
          messageId: "muiTopLevelImportError",
          fix(fixer) {
